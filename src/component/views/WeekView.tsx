...
export default function WeekView({ currentDate, events, onEventClick, onDateEventsClick, loading }: WeekViewProps) {
  ...
  const calculateDynamicHourHeight = (date: Date, hour: number) => {
    ...
    return Math.round(BASE_HOUR_HEIGHT + additionalHeight)
  }

  const calculateEventHeight = (event: CalendarEvent, hourHeight: number) => {
    const durationPercentage = calculateEventDurationPercentage(event)
    const availableSpace = hourHeight - 40
    const baseEventHeight = 70
    const inversePercentage = 1 - durationPercentage
    const eventHeight = baseEventHeight + (availableSpace - baseEventHeight) * inversePercentage

    return Math.max(60, eventHeight)
  }

  const calculateEventTopOffset = (event: CalendarEvent, hour: number, hourHeight: number) => {
    const durationPercentage = calculateEventDurationPercentage(event)
    const spaceOffset = (1 - durationPercentage) * 30
    ...
    if (eventHour === hour) {
      return (eventMinutes / 60) * hourHeight + spaceOffset
    }
    return spaceOffset
  }
                  ...
                  >
                    <div className="absolute inset-0 p-2">
                      {groupedEvents.map((group, groupIndex) => {
                        if (!isEventStartHour(group.events[0], hour)) return null

                        const event = group.events[0]
                        const eventHeight = calculateEventHeight(event, maxHourHeight)
                        ...
                        return (
                          <div
                            ...
                            style={{ top: `${topOffset}px`, height: `${eventHeight}px`, zIndex: 10 }}
                          >
                            {group.events.length === 1 ? (
                              ...
                              <div
                                ...
                                className="relative cursor-pointer h-full mt-1"
                              >
                                <EventCard
                                  event={event}
                                  onClick={() => { }}
